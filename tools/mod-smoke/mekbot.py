"""Joins, breaks a prepared Mekanism ore and storage block, idles for pickups."""
    ...
    if state == "login" and pid == 2:
        ...
        if pid == 72:
            ...
            if not acted:
                acted = True
                def act():
                    time.sleep(25)  # let the tp + setblocks land
                    seq = 1
                    for (x, y, z) in ((0, 150, -1), (1, 150, -1)):
                        # survival: start destroy, mine out the hardness, finish
                        send(sock, 41, (varint(0) + encode_blockpos(x, y, z)
                                        + struct.pack("B", 1) + varint(seq)))
                        seq += 1
                        time.sleep(12)
                        send(sock, 41, (varint(2) + encode_blockpos(x, y, z)
                                        + struct.pack("B", 1) + varint(seq)))
                        seq += 1
                        print(f"BOT: broke {x},{y},{z}", flush=True)
                        time.sleep(2)
                    time.sleep(10)
