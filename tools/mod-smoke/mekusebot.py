"""Joins, right-clicks the prepared energized smelter, reports any screen."""
...
while time.time() < deadline:
    ...
    if state == "login" and pid == 2:
        ...
        if pid == 44 and len(body) == 8:
            send(sock, 28, body)
        if pid == 59:
            wid, pos = read_varint_buf(body, 0)
            mtype, pos = read_varint_buf(body, pos)
        ...
        if pid == 72:
            ...
            if not acted:
                ...
                def act():
                    ...
                    # use_item_on: hand 0, block (2,150,-1), face 1(top), cursor, seq
                    send(sock, 66, (varint(0) + encode_blockpos(2, 150, -1) + varint(1)
                                    + struct.pack(">fff", 0.5, 1.0, 0.5) + b"\x00" + b"\x00" + varint(1)))
                    print("BOT: clicked smelter", flush=True)
                    time.sleep(8)
