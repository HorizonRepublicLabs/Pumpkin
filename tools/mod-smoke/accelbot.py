"""Joins, places a growth accelerator on the prepared stone, idles while it ticks."""
...
state = "login"
acted = False
IDLE = int(os.environ.get("ACCEL_IDLE", "240"))
    ...
    if state == "login" and pid == 2:
        ...
        if pid == 72:
            ...
            if not acted:
                acted = True
                def act():
                    time.sleep(10)  # stone + give landed
                    send(sock, 53, struct.pack(">h", 0))
                    time.sleep(0.5)
                    # place the accelerator on top of the stone at (0,147,-1)
                    send(sock, 66, (varint(0) + encode_blockpos(0, 147, -1) + varint(1)
                                    + struct.pack(">fff", 0.5, 1.0, 0.5) + b"\x00" + b"\x00"
                                    + varint(1)))
                    print("BOT: accelerator placed", flush=True)
                    time.sleep(IDLE)
