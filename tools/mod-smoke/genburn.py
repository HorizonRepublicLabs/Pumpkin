...
BLOCKS = [
    "setblock 8 150 0 mekanismgenerators:heat_generator",
]
...
INSERTS = {}
TANK_INSERTS = {
    b"mekanismgenerators:heat_generator": {"items": [{"stored": {
        "resource": "minecraft:coal", "amount": 64}, "container": 0}]},
}
...
for path in glob.glob(os.path.join(srv, "world/**/region/*.mca"), recursive=True):
    ...
    for i in range(1024):
        ...
        for e in walk_block_entities(root):
            if e[0] != "compound": continue
            _, ident = compound_get(e, b"id")
            _, blob = compound_get(e, b"pumpkin:mod_data")
            if ident and ident[1] == b"mekanismgenerators:heat_generator" and blob is not None:
                burned = json.loads(base64.b64decode(blob[1]))
print("GEN STATE:", json.dumps(burned)[:400] if burned else "no blob")
sys.exit(0)
